...
def patch_rand_drop2D(local_rank, x, x_rep=None, max_drop=0.3, max_block_sz=0.25, tolr=0.05):
    c, h, w = x.size()
    ...
    while total_pix < n_drop_pix:
        ...
        if x_rep is None:
            x_uninitialized = torch.empty(
                (c, rnd_h - rnd_r, rnd_w - rnd_c), dtype=x.dtype, device=local_rank).normal_()
...
def aug_rand(local_rank, samples):
    img_n = samples.size()[0]
    x_aug = samples.detach().clone()
    for i in range(img_n):
        x_aug[i] = patch_rand_drop2D(local_rank, x_aug[i])
        idx_rnd = randint(0, img_n)
        if idx_rnd != i:
            x_aug[i] = patch_rand_drop2D(local_rank, x_aug[i], x_aug[idx_rnd])
    return x_aug
